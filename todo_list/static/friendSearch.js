...
document.querySelectorAll('.friend-add').forEach(btn => {
    btn.addEventListener('click', e => {
        fetch(`./friends/add?user=${e.target.getAttribute('add-user')}`)
        .then(response => {
            if (response.status === 200) {
                let parentDiv = e.target.parentNode;
                parentDiv.innerHTML = "";
                parentDiv.insertAdjacentHTML('beforeend', '<span class="pending">Solicitud pendiente</span>');
                e.target.remove();
            }
        });
    });
